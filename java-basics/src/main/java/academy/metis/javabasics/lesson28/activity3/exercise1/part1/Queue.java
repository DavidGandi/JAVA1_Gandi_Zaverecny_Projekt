...
public class Queue {
    ...
    public void addToQueue(long memberId, long titleId) {
        ...
        if (rentedTwoTitlesOrRentedChosenTitleAlready(memberId, titleId)){
            scanner.nextLine();
            return;
        }
    ...
    //validacia poctu vypoziciek / zhoda titulov v queueRecords
    private boolean memberAlreadyRentedThisTitle(QueueRecord queueRecord, long titleId) {

        return queueRecord.getTitleId() == titleId && !queueRecord.isResolved();
    }

    private boolean rentedTwoTitlesOrRentedChosenTitleAlready(long memberId, long titleId) {

        int activeRentalsInQueue = 0;

        for (QueueRecord queueRecord : Queue.queueRecords.values()){
            if (queueRecord.getMemberId() == memberId){
                if (memberAlreadyRentedThisTitle(queueRecord, titleId)){
                    System.out.println("You can not add this title to queue because you have already added this title to queue.");
                    return true;
                }
                if (!queueRecord.isResolved()){
                    activeRentalsInQueue++;
                }
            }
        }
        if (activeRentalsInQueue >= 2){
            System.out.println("You can not add this title to queue because you are already in queue for two titles.");
            return true;
        }
        return false;
    }
